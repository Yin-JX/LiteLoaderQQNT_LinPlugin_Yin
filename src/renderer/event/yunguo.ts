...
class Yunguo extends BaseEvent {
  ...
  private get isAtUid() {
    const arr = this.message.elements.filter(
      (e) => e.elementType === 1 && e.textElement.atType === 2
    );
    return arr.some(
      (e) => e.textElement.atUid === String(this.globalData.selfUin)
    );
  ...
  async onBoss() {
    ...
    if (this.isAtSelf || isSs) {
      ...
      const tuan_s = this.getNumberValue(
        this.markdownElementContent.match(/又不会跑，还有(\d+)秒冷却/)?.[1]
      );
    ...
    if (this.isAtUid) {
      if (
        ...
        if (kuileiCmdTemp.match(/团本斩杀线(\d+)/)) {
          linPluginAPI.setConfig(
            "tbzsx",
            kuileiCmdTemp.match(/团本斩杀线(\d+)/)?.[1]
          );
          傀儡.send信号(
            `分路为${this.config.tbfl},斩杀线已设置为 ${
              kuileiCmdTemp.match(/团本斩杀线(\d+)/)?.[1]
            },若需要更改请发送"团本斩杀线xx","路xx"`
          );
        } else if (kuileiCmdTemp.match(/路(\d+)/)) {
          linPluginAPI.setConfig("tbfl", kuileiCmdTemp.match(/路(\d+)/)?.[1]);
          傀儡.send信号(
            `分路为${kuileiCmdTemp.match(/路(\d+)/)?.[1]},斩杀线已设置为 ${
              this.config.tbzsx
            },若需要更改请发送"团本斩杀线xx","路xx"`
          );
          ...
          await linPluginAPI.setConfig(
            "faCheCmd",
            "确定发起公会组队" + kuileiCmdTemp.match(/公会(\d+)/)?.[1]
          );
          傀儡.wusendCmd(
            `目标副本指令已切换为 确定发起公会组队 ${
              kuileiCmdTemp.match(/公会(\d+)/)?.[1]
            },若需要更改请发送"公会xx"`
          );
          ...
          await linPluginAPI.setConfig(
            "发车时间",
            kuileiCmdTemp.match(/发车时长(.+)/)?.[1]
          );
          傀儡.wusendCmd(
            `当前发车时长为 ${
              kuileiCmdTemp.match(/发车时长(.+)/)?.[1]
            } * 5 + 8 = ${
              Number(kuileiCmdTemp.match(/发车时长(.+)/)?.[1]) * 5 + 8
            }秒,若需要更改请发送"发车时长xx"`
          );
        } else if (kuileiCmdTemp.match(/团本指令(.+)/)) {
          linPluginAPI.setConfig(
            "团本指令",
            kuileiCmdTemp.match(/\*团本指令(.+)/)?.[1]
          );
          傀儡.send信号(
            `当前团本指令为 ${
              kuileiCmdTemp.match(/\*团本指令(.+)/)?.[1]
          ...
          傀儡.wusendCmd(
            `配置信息=>普通本和公会本这边，我目前${
              this.config.autoFaCheFlag ? "是" : "不是"
            } 车头,\n
						${this.config.diaoShuiAutoFaCheFlag ? "使用" : "不使用"} 打完副本掉水续车,\n
						${this.config.自动跟车Flag ? "并且我" : ""}${
              this.config.自动跟车Flag ? "会" : "不会"
            } 自动在 ${
              this.config.跟车次序
                ? this.config.跟车次序 + "(带车头)"
                : "(啊这，我没有上车次序哦)"
            } 号位跟车,\n
						跟车间隔为 ${Number(this.config.跟车间隔)}毫秒,\n
						副本指令: ${this.config.faCheCmd},\n
						发车间隔为 ${Number(this.config.发车时间 * 5 + 8)}秒。\n
						团本方面，我目前分路为 ${this.config.tbfl},斩杀线为 ${
              this.config.tbzsx / 10000
            } 万滴血\n
        ...
        if (kuileiCmdTemp.match(/\*你来开团/)) {
          ...
          傀儡.send信号(
            `好的，现在由我来开团，请注意关闭其他开团手，若需要关闭请at对应目标发送"*关团"，并且，当前由于我作为开团手，我的上团次序已清空，请注意让其他人补位。上团次序为 ${this.config.总次序} ，若需要更改其他人次序请发送"*次序xx"`
          );
          linPluginAPI.setConfig("总次序", null);
        } else if (kuileiCmdTemp.match(/\*关团/)) {
          linPluginAPI.setConfig("发团人flag", false);
          傀儡.send信号(
            `已关闭开团手，若需要开启请at对应目标发送"*你来开团"，当前并无上团次序，若需要安排请发送"*次序xx"`
          );
          linPluginAPI.setConfig("总次序", null);
        } else if (kuileiCmdTemp.match(/\*次序(\d+)/)) {
          linPluginAPI.setConfig(
            "总次序",
            kuileiCmdTemp.match(/\*次序(\d+)/)?.[1]
          );
          傀儡.send信号(
            `当前上团次序为 ${
              kuileiCmdTemp.match(/次序(\d+)/)?.[1]
            },若需要更改请at对应目标发送"次序xx",请注意不要和别人冲突，同时为了避免产生冲突，默认安排次序时会关掉开团手职位，若需要开启请at对应目标发送"*你来开团"`
          );
          linPluginAPI.setConfig("发团人flag", false);
          ...
          傀儡.send信号(
            kuileiCmdTemp.match(/\-人机/) ? "你tm才人机" : "智nm障"
          );
          ...
          var 回复 = [
            "?",
            `${
              kuileiCmdTemp.includes("普攻")
                ? "我打boss？！真的假的……"
                : kuileiCmdTemp.match(/放弃(.+)/)?.[1]
                ? "永不言弃!!"
                : "我不要"
            }`,
            `${
              kuileiCmdTemp.includes("确定") ? "对，我确定" : "嗯？好像不太确定"
            }`,
            "??",
            "是这样的",
            "啊？",
            `${kuileiCmdTemp
              .substring(
                0,
                this.取随机整数(
                  kuileiCmdTemp.length * 0.2,
                  kuileiCmdTemp.length * 0.8
                )
              )
              .split("")
              .reverse()
              .join("")}……？还是什么东西？`,
            "怎么了？",
            `什么${kuileiCmdTemp.substring(
              0,
              kuileiCmdTemp.length * 0.2
            )}…什么${kuileiCmdTemp.substring(
              kuileiCmdTemp.length * 0.2 + 1,
              kuileiCmdTemp.length * 0.7
            )}……然后什么${kuileiCmdTemp.substring(
              kuileiCmdTemp.length * 0.7,
              kuileiCmdTemp.length
            )}？？`,
            "额",
            "..",
            "....",
            ...
            `什么${kuileiCmdTemp}？`,
            `${kuileiCmdTemp}干嘛？`,
            `${kuileiCmdTemp.split("").reverse().join("")}`,
            `啊？${kuileiCmdTemp.substring(
              this.取随机整数(0, kuileiCmdTemp.length * 0.3),
              this.取随机整数(
                kuileiCmdTemp.length * 0.3,
                kuileiCmdTemp.length * 0.6
              )
            )}……？？`,
          ];
  ...
  async onZLC() {
    ...
    const 当前抓捕 =
      this.markdownElementContent.match(/当前抓捕(:|：)(.+)/)?.[2];
    ...
    const 抓捕率 = Number(
      this.markdownElementContent.match(/抓捕成功率(:|：)(\d+)%/)?.[2]
    );
    const 灵宠行动冷却 = Number(
      this.markdownElementContent.match(/请等待(\d+)秒再次操作/)?.[1]
    );
    const 已抓捕 = this.markdownElementContent.includes("恭喜你");
    const 抓捕失败 = this.markdownElementContent.includes("跑掉了");

    const 是自己 =
      this.markdownElementContent.match(/用户(:|：)(\d.+)/)?.[2] ===
      this.config.yunGuoUid
        ? true
        : false;
  ...
  async 接受2() {
    ...
    if (this.config.全局监控指令) {
      ...
      if (消息.match(/at长生(.+)/) && !this.isAtUid) {
        ...
        接受2.klsendCmd(
          this.config.yaoshuiCmd ? this.config.yaoshuiCmd : "升级"
        );
      } else if (消息.match(/-(清理标记|清)/)) {
        消息组.group = [];
        消息组.text = [];
        接受2.send信号("标记已清理");
      } else if (消息.match(/-全局(关闭|开启)/)) {
        const 类别 =
          消息.match(/-全局(关闭|开启)/)?.[1] === "开启" ? "开启" : "关闭";
        if (类别 === "开启") {
          全局关闭 = false;
          接受2.send信号("已开机");
        } else if (类别 === "关闭") {
  ...
  async TB2() {
    ...
    const 团本开始标志 =
      this.markdownElementContent.match(/开始了团本挑战副本/);
    const 阶段1标志 =
      this.markdownElementContent.match(/部位/) ||
      团本阶段一部位列表.includes(死亡元素)
        ? true
        : false;
    const 阶段2标志 = this.markdownElementContent.match(/完整/);
    const 本路击败 =
      团本阶段一部位列表.includes(击败元素) && this.isAtSelf ? true : false;
    const 完整体击败 = 团本完整体列表.includes(击败元素);

    const 发车的冷却 = Number(
      this.markdownElementContent.match(/你还有(\d+)秒冷却才能发起组队/)?.[1]
    );
    const 团本指令 = this.config.团本指令;
    const 发团人flag = this.config.发团人flag;
    ...
    const 技能点 = Number(
      this.markdownElementContent.match(/技能点(:|：)(\d+)/)?.[2]
    );
    const 不足 = this.markdownElementContent.includes("技能点不足");
    const 禁锢秒 =
      Number(
        this.markdownElementContent.match(/禁锢了你 (\d+) 秒/)
          ? this.markdownElementContent.match(/禁锢了你 (\d+) 秒/)?.[1]
    ...
    if (tbFlag && !蹭) {
      ...
      if (阶段1标志 && this.isAtSelf) {
        const 阶段一血量 = Number(
          团本团队标志[0].match(/血量(:|：)(\d+)/)?.[2]
            ? 团本团队标志[0].match(/血量(:|：)(\d+)/)?.[2]
            : this.markdownElementContent.match(/血量(:|：)(\d+)/)?.[2]
        );
        if (!斩杀线信号.every((element) => element === true)) {
          //阶段一并没有全路到斩杀线
      ...
      //阶段2
      if (
        阶段一被打死信号.every((element) => element === true) &&
        !打死阶段1首次触发信号 &&
        (分路 == 2 || 分路 == 1)
      ) {
        //准备进入阶段2
